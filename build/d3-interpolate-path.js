...
var commandTokenRegex = /[MLCSTQAHVZmlcstqahv]|-?\d*\.?[\de+]*/g;
/**
...
function pathCommandsFromString(d) {
  ...
  for (var i = 0; i < tokens.length; ++i) {
    commandArgs = typeMap[tokens[i]]; // if commandArgs was invalid, it may be encounering an "implicit repeated command" where the args are directly after the previous command
    ...
    if (commandArgs) {
      command = {
        type: isRepeatedCommand ? command.type : tokens[i]
