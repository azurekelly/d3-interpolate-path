...
var commandTokenRegex = /[MLCSTQAHVZmlcstqahvz]|-?(\d+\.|\.)?\d+([Ee][-+]?\d+)?/g;
/**
...
function pathCommandsFromString(d) {
  ...
  var commandArgs;
  var command = {};
  var isRepeatedCommand = false; // iterate over each token, checking if we are at a new command
  // by presence in the typeMap

  for (var i = 0; i < tokens.length; ++i) {
    commandArgs = typeMap[tokens[i]]; // if commandArgs was invalid, it may be encountering an "implicit repeated command" where the args are directly after the previous command,
    ...
    if (commandArgs) {
      if (isRepeatedCommand) {
        command = {
          type: // when an M command is repeated it's considered a L command by the SVG spec
          // all other command types stay the same upon repeating
          command.type === 'M' ? 'L' : command.type === 'm' ? 'l' : command.type
        };
      } else {
        command = {
          type: tokens[i]
      ...
      for (var a = 0; a < commandArgs.length; ++a) {
        var token = tokens[i + a]; // handle implicit 0 before decmial point
